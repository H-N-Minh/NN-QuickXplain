...
def get_camera_diagnosis(test_file_path=None, variable_order_file_path=None):
    try:
        if variable_order_file_path:
            result = subprocess.run(["java", "-jar",
                                     r"C:\Users\mathi\Documents\Studium\Promotion\MF4ChocoSolver-main\ConfigurationChecker\camera_diagnosis.jar",
                                     test_file_path, variable_order_file_path],
                                    capture_output=True, text=True, timeout=400)
        else:
            result = subprocess.run(["java", "-jar",
                                     r"C:\Users\mathi\Documents\Studium\Promotion\MF4ChocoSolver-main\ConfigurationChecker\camera_diagnosis.jar",
